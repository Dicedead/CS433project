...
cos_p_model = CosParentGenerator.load("../model_parameters/water/no_em_cos_p_prediction.sav",
                                          "../model_parameters/water/no_em_cos_p_prediction_dataset_stats")

def predict_cos_p(p: Particle, distance: float):
    return cos_p_model.generate_from_particle(p, distance)[0]


values = np.array([predict_cos_p(Particle(0, 0, 0, 0, 0, 0, energies[x], Type.photon), distances[x])
